 ...
 */
public class MatcherCharlie extends BaseRichBolt {

    private static final Logger LOG = LoggerFactory.getLogger(MatcherCharlie.class);
...
//        loadIndex(context.getThisTaskIndex(), context.getComponentTasks(context.getThisComponentId()).size());
    ...
    public void execute(Tuple input) {
        ...
        int groupCnt = input.getIntegerByField(SEND_GROUP_CNT);
        ...
        collector.emit(STREAM_MATCH_IMAGES, input, new Values(frameId, matches, feaDescCnt, groupCnt));
        collector.ack(input);
    }
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(STREAM_MATCH_IMAGES, new Fields(FIELD_FRAME_ID, FIELD_MATCH_IMAGES, FIELD_FEATURE_CNT, SEND_GROUP_CNT));
    }
